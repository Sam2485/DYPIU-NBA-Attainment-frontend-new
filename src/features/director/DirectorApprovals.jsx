...
export default function DirectorApprovals() {
  ...
  const [approvalList, setApprovalList] = useState(directorApprovals);
  const [remarksMap, setRemarksMap] = useState({});

  const handleApprove = (appId) => {
    const updated = approvalList.map((a) =>
      a.id === appId
        ? {
            ...a,
            status: 'APPROVED',
            approvedBy: user?.name || 'School Director',
            approvedAt: new Date().toISOString().split('T')[0],
          }
        : a
    );
    setApprovalList(updated);
